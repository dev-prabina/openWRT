...
return baseclass.extend({
	...
	renderMainMenu(tree, url, level) {
		...
		children.forEach(child => {
			...
			const hasSubmenu = submenu.firstElementChild != null;
			const isCurrent = (L.env.dispatchpath.length > (1 + (level || 0))) && (L.env.dispatchpath[1 + (level || 0)] === child.name);
			
			let liClasses = [];
			if (!level && hasSubmenu) liClasses.push('dropdown');
			if (isCurrent) liClasses.push('active');

			const linkclass = (!level && hasSubmenu) ? 'menu' : '';
			const linkurl = hasSubmenu ? '#' : L.url(url, child.name);
			const li = E('li', { 'class': liClasses.join(' ') }, [
